...
\begin{definition}
  ...
  \]
  Fehlt noch der Nachweis der Wohldefiniertheit:
...
\begin{definition}
  Die \begriff{Potenzfunktion}\index{$l^{(\_)}$} $l^{(\_)}:\Z\to\Omega(S^1,\ast)$ ist $\Z'$- und $\N_1$-rekursiv gegeben durch
  \begin{align*}
...
Anstatt direkt zu zeigen, dass $l^{(\_)}$ und $w$ zueinander invers sind, werden wir eine faserweise Äquivalenz der Form
\[
...
\begin{beweis}
  ...
  \[
    P_x(W_x(p))=p
  \]
  Dank Induktion mit Basispunkt müssen wir das nur für $p\equiv \refl_*$ berechnen:
  \[
    P_\ast(W_\ast(\refl_*))=P_\ast(0)=\refl_*
  \]
...
\begin{lemma}
  ...
  \begin{center}
    \begin{tikzcd}
      \Omega A \ar[r]\ar[d,"\Delta"] & \eins\ar[d] \\
...
\begin{beweis}
  ...
  \begin{center}
    \begin{tikzcd}[column sep=2cm,row sep=large]
      (X\to B^nK)\ar[r]\ar[d,swap,"{(\_\circ\iota_1,\_ \circ \iota_2)}"] &(C\to B^nK)\ar[d]\ar[r] & \eins\ar[d]  \\
      (A\to B^nK)\times (B\to B^nK)\ar[r,"(\_\circ f) \times (\_ \circ g)"] & (C\to B^nK)\times (C\to B^nK)\ar[r,"d"] & (C\to B^nK)
    \end{tikzcd}
  \end{center}
...
\begin{beweis}[Idee]
  ...
  \end{enumerate}
  (a) ist \Cref{bsp:kohom} (c).

  Für $k\equiv 0$ ergibt sich $H^n(S^0,\Z)\to H^{n+1}(S^{1},\Z)$. Für $n>0$ ist aber $H^n(S^0,\Z)\equiv H^n(\zwei,\Z)\simeq\eins$ nach \Cref{bsp:kohom} (b), was (b) zeigt.

  Schließlich folgt (c) aus \Cref{bem:Sn-zsh}, also daraus, dass $S^{k}$ $1$-zusammenhängend und $S^1$ $1$-abgeschnitten ist:
  \begin{align*}
